...
const EditForm: React.FC<Props> = ({singleNews}) => {
    ...
    const [subject, setSubject] = useState(singleNews?.subject)
    const [content, setContent] = useState<string | null>(singleNews?.content ?? '')
    const [expireDate, setExpireDate] = useState(singleNews?.expireDate)
    ...
    return (
        <form action={action} className='w-full h-full'>
            <input type="hidden" id='id' name='id' value={singleNews?.id}/>
            <div className="mb-1">
